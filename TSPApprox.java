...
class TSPApprox extends TSPAlg {
	...
	public void doNextStep() {
		// YOUR CODE HERE
		HashMap<Flight, Double> edgeMap = null;

		edgeMap = fillEdgeMap(graph);

		MinimumSpanningForest<City, Flight> mst = new MinimumSpanningForest<>(graph, new DelegateForest<>(),
				startingCity, edgeMap);
		tree = mst.getForest();
		switch (step) {
			case STEP_MST:
			...
			case STEP_DFT:
				...
				depthFirstTraversal(tree, startingCity, visitOrder);
				computeVisitOrderCost(graph, visitOrder, startingCity);
				break;
	...
	private void computeVisitOrderCost(Graph<City, Flight> graph, List<City> visitOrder, City startingCity) {

		int totalCost = 0;

		for (int i = 0; i < visitOrder.size(); i++) {
			if (i == visitOrder.size() - 1) {
				Flight flight = graph.findEdge(visitOrder.get(i), startingCity);
				totalCost += flight.getCost();
			} else {
				Flight flight = graph.findEdge(visitOrder.get(i), visitOrder.get(i + 1));
				totalCost += flight.getCost();
			}
		}
	...
	/**
	 * Creates a HashMap to store the flights and their costs.
	 * 
	 * @param graph containing Cities and flights to other cities.
	 * @return HashMap mapping Flights -> Cost
	 */
	private HashMap<Flight, Double> fillEdgeMap(Graph<City, Flight> graph) {
		HashMap<Flight, Double> map = new HashMap<>();
		List<Flight> edges = new ArrayList<>(graph.getEdges());

		for (Flight edge : edges) {
			map.put(edge, (double) edge.cost);
		}
	...
	static void depthFirstTraversal(Forest<City, Flight> treeToWalk, City currNode, List<City> dftVisitOrder) {
		...
		if (!dftVisitOrder.contains(currNode)) {
			dftVisitOrder.add(currNode);

		}

		for (City city : treeToWalk.getSuccessors(currNode)) {

			depthFirstTraversal(treeToWalk, city, dftVisitOrder);
		}
